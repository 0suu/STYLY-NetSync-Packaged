...
class NetSyncForegroundService : Service() {
    ...
    @Volatile
    private var state: Int = ServerState.STOPPED
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START -> {
                val selectedHost = intent.getStringExtra(EXTRA_HOST).orEmpty().ifBlank { "0.0.0.0" }
                val selectedPort = intent.getIntExtra(EXTRA_PORT, 5555)
                startServer(selectedHost, selectedPort)
            }
    ...
    private fun startServer(selectedHost: String, selectedPort: Int) {
        val currentState = state
        if (currentState == ServerState.RUNNING || currentState == ServerState.STARTING) {
            Log.i(TAG, "Server already running/starting")
            return
        }
        if (selectedPort != 5555) {
            Log.w(TAG, "Port $selectedPort selected. Unity NetSync default client expects dealer=5555 and sub=5556.")
        }

        state = ServerState.STARTING

        ensureNotificationChannel()
        startForeground(NOTIFICATION_ID, buildNotification(selectedHost, selectedPort))
        acquireLocks()
        ...
        executor.execute {
            try {
                ...
                pyBootstrap?.callAttr("start", selectedHost, selectedPort, logPath)

                state = ServerState.RUNNING
                Log.i(TAG, "Server started at $selectedHost:$selectedPort")
                refreshNotification(selectedHost, selectedPort)
            } catch (t: Throwable) {
    ...
    private fun buildNotification(selectedHost: String, selectedPort: Int): Notification {
        ...
        val builder = NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .setContentText("$selectedHost:$selectedPort")
    ...
    private fun refreshNotification(selectedHost: String, selectedPort: Int) {
        val notification = buildNotification(selectedHost, selectedPort)
        val manager = getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
